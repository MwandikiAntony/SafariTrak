...
foreach ($allNotifications as &$n) {
    ...
    if ($n['type'] === 'group_journey_invite') {
        ...
        $groupJourneyId = isset($n['related_journey_id'])
            ? (int)$n['related_journey_id']
            : 0;
...
.group-invite-actions .accept-invite {
    background: #198754;
    color: #fff;
}

.group-invite-actions .decline-invite {
    background: #dc3545;
    color: #fff;
}

.group-invite-actions button:disabled {
    opacity: .6;
    cursor: not-allowed;
}
...
.group-invite-status.success {
    color: #198754;
}

.group-invite-status.error {
    color: #dc3545;
}
</style>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav>
        ...
        <a href="messages.php">
            ...
            Messages<?= $unreadConversationCount > 0 ? " <em>" . $unreadConversationCount . "</em>" : "" ?>
        </a>
...
<div class="page-head">
    ...
    <div>
        ...
        <p>
            Journey updates, messages, group invitations,
            and safety alerts all in one place.
        </p>
    </div>
...
<?php if (empty($allNotifications)): ?>
    ...
    <p
        class="hint"
        style="padding:24px 21px;color:var(--muted);font-size:11px"
    >
        Nothing here yet. Journey updates, messages,
        group invitations and safety alerts will show up here.
    </p>
...
<?php foreach ($groups as $label => $items): ?>
    ...
    <div
        class="notif-page-list"
        data-group="<?= strtolower(str_replace(' ', '-', $label)) ?>"
    >
    ...
    <?php foreach ($items as $n): ?>
        ...
        if ($isGroupInvite) {
            ...
            $link = st_notif_link(
                $n['type'],
                $n['related_journey_id'] ?? null,
                $n['related_user_id'] ?? null
            );
        }

        $tag = $link ? 'a' : 'div';

        $sosClass = $n['type'] === 'sos_alert'
            ? ' sos'
            : '';

        $unreadClass = $n['is_read']
            ? ''
            : ' unread';
        ...
        <<?= $tag ?>
            ...
            class="notif-row<?= $unreadClass . $sosClass ?>"
            data-type="<?= htmlspecialchars(st_notif_category($n['type'])) ?>"
            data-id="<?= (int)$n['id'] ?>"
            ...
            <div class="ninfo">
                ...
                <?php if ($isGroupInvite && $n['group_invite_id']): ?>
                    ...
                    <?php if ($n['group_inviter_name']): ?>
                        ...
                        <p>
                            <strong>
                                <?= htmlspecialchars($n['group_inviter_name']) ?>
                            </strong>
                    ...
                    <div
                        ...
                        data-invite="<?= (int)$n['group_invite_id'] ?>"
                    >

                        <button
                            type="button"
                            class="accept-invite"
                            data-invite-id="<?= (int)$n['group_invite_id'] ?>"
                            data-action="accept"
                        >
                    ...
                    <div
                        class="group-invite-status"
                        id="invite-status-<?= (int)$n['group_invite_id'] ?>"
                    ></div>
                ...
                <small>
                    <?= (new DateTime($n['created_at']))->format('g:i A, j M') ?>
                </small>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const acceptButtons = document.querySelectorAll('.accept-invite');
    const declineButtons = document.querySelectorAll('.decline-invite');
    ...
    function respondToInvitation(button) {
        ...
        .then(function (data) {
            ...
            if (data.success) {
                ...
                statusBox.textContent =
                    data.message || 'Unable to process invitation.';
